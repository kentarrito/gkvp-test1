...
\usepackage[top=35truemm,bottom=20truemm,left=30truemm,right=30truemm]{geometry}
...
\hypersetup{
  ...
  citecolor=blue,
  urlcolor=blue,
  pdftitle={Training Large Language Models for Fusion Plasma Simulation Code Development},
}

% ---- Title info ----
\title{Training Large Language Models for Fusion Plasma Simulation Code Development}
\author{Kentaro Seki\\
Kyoto University\\
...
Section 4 shows the detailed experimental setups and results.

Section 5 discusses the experimental results presented in the previous section and unsuccessful attempts we made through the experiments.

Finally, we conclude this thesis in section 6.
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.95\linewidth]{seimei_idea.png}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.95\linewidth]{rmsearch_idea.png}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.95\linewidth]{image-10.png}
  \caption*{\textbf{Figure 7 | Preference-style dataset creation for DPO training.}}
\end{figure}
...
Figure 9 shows the evaluation result of testing SEIMEI pipeline without RMSearch and with trained RMSearch of iteration 1 and 2. Compared with the base pipeline, the SEIMEI pipeline augmented by RMSearch shows consistent gains. Accuracy improves by roughly 7\% in each iteration, and by the second iteration the cumulative improvement exceeds 14\%. This suggests that better retrieval and reranking translate into tangible improvements in end-to-end repair performance.
...
In this section, we discuss the experimental results presented in section 4. In section 5.1, we explain how to interpret the results and give insights into the ability of the framework developed in this study. As we have arrived at the final framework by trial and error, we describe several modifications in section 5.2. Also, the advantages of the final framework will be presented in section 5.3.
...
% =========================
% Replace your current Conclusion section with the following
% =========================
\section{Conclusion}

This thesis proposed and evaluated a practical framework for \textbf{domain-adapting an LLM-based coding assistant to a single, specialized scientific codebase} without retraining the base generator model. Focusing on turbulent transport simulation for fusion plasmas, we used the open-source \textbf{gyrokinetic Vlasov code GKV} as a realistic target where correctness is physics-sensitive and code understanding requires repository-scale context.

Our first contribution is the construction of a \textbf{break-and-repair dataset} tailored to scientific computing. By intentionally removing physics-critical lines and pairing each deletion with a ground-truth patch, we obtained training and test problems with an unambiguous notion of correctness. This dataset design is especially valuable for scientific software, where evaluation is often difficult: by anchoring tasks to known patches, we can measure repair success beyond surface-level plausibility and directly connect model behavior to code semantics.

Our second contribution is an \textbf{agentic inference pipeline} implemented with SEIMEI, which structures repository exploration (file listing, reading, keyword search, and step planning) and produces candidate patches with intermediate traces. Rather than relying purely on prompt engineering, SEIMEI treats code repair as a multi-step process that can be logged, scored, and improved, making it suitable for iterative development and analysis.

The core contribution of this thesis is demonstrating that \textbf{retrieval/routing can be the main learnable component} for domain adaptation. We constructed a \textbf{knowledge pool} from both manual heuristics and automatically distilled lessons from solved repairs, and trained \textbf{RMSearch} as a reranker-style model using \textbf{DPO-style preference optimization}. The reward design explicitly ties training signals to end-to-end repair quality (correct file, correct location, functional restoration, and measured knowledge contribution), and the batching strategy follows the practical insight from reward-model training that within-query comparisons should be treated as a structured unit.

Experiments on 113 repair tasks show consistent trends: RMSearch evaluation loss decreases and accuracy increases across iterations, and when RMSearch is integrated into SEIMEI, \textbf{end-to-end repair accuracy improves by roughly 7\% per iteration}, exceeding \textbf{14\% cumulative improvement by the third iteration} compared to a baseline pipeline without RMSearch-based routing. Importantly, these gains were achieved at \textbf{modest computational cost} (on the order of a few GPU-hours per stage and approximately \$6 per iteration in our Runpod setting), supporting the thesis’s central claim that \emph{lightweight domain adaptation via reranking is feasible for academic-scale resources}.

There are clear limitations. The current evaluation is restricted to a single codebase and a synthetic break-and-repair formulation, and the reward relies on comparison to ground-truth patches rather than fully physics-based runtime validation. In addition, knowledge extraction quality can vary: noisy or overly general “lessons” may reduce usefulness as the knowledge pool scales, suggesting the need for stronger filtering and validation gates.

Future work naturally extends along three directions. First, we will evaluate generality by expanding to additional gyrokinetic and plasma-physics repositories and by incorporating more realistic defect sources (e.g., refactoring mistakes, boundary-condition regressions, and diagnostic inconsistencies). Second, we will strengthen correctness checks using compilation/tests and, where feasible, physics-facing validation signals (e.g., conservation properties or regression metrics on representative runs). Third, we will improve the knowledge lifecycle: automated distillation, deduplication, and confidence scoring, combined with periodic RMSearch updates, would move the system toward a \textbf{continuous, low-cost learning loop} where the base LLM remains stable while the search model and external memory evolve with the repository.

Overall, this thesis provides evidence that \textbf{domain-specific retrieval and preference-trained reranking} can materially improve scientific code repair, offering a concrete path toward reliable LLM assistance for complex simulation software under realistic compute constraints.

% =========================
% Replace your current Acknowledgements section with the following
% =========================
\section*{Acknowledgements}
\addcontentsline{toc}{section}{Acknowledgements}

I would like to express my sincere gratitude to \textbf{Ms.\ Narita} for her guidance and support throughout this project. Her careful feedback helped me refine both the experimental design and the writing, and her encouragement made it possible to continue iterating through failures and improvements until the system became measurable and stable.

I am also deeply thankful to my colleagues at \textbf{KyotoAI Inc.}---\textbf{Saxena Prakhar}, \textbf{Juan Pablo}, \textbf{Mingkwan Rattanasiwamok}, and \textbf{Roshia}---for sharing foundational ideas and practical insights on RMSearch and for the many discussions that clarified what matters in real engineering workflows: reproducibility, careful logging, and evaluation that connects model behavior to outcomes. Their day-to-day support and willingness to answer small questions made a substantial difference in my progress.

I would like to acknowledge the \textbf{GKV developers} and the broader fusion and gyrokinetics research community for producing and maintaining high-quality scientific software and publications. Open repositories and well-documented methods are essential for making research in this area reproducible and extensible, and this thesis was only possible because such resources are publicly available.

I also appreciate the creators and maintainers of the open-source tools and libraries that supported this work, as well as the infrastructure providers that enabled practical experimentation under limited time and hardware constraints, including the GPU resources used for inference and training in our iterative runs.

Finally, I would like to thank my family for their constant support. Their patience and encouragement sustained me through long debugging sessions, repeated experimental iterations, and the writing process. This work is the result of that support as much as it is of the experiments described in the thesis.
