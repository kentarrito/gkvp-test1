...
What is a token?\\
LLMs do not read text as characters or “words” directly. Instead, input is converted into a sequence of \textbf{tokens}, which are discrete units produced by a tokenizer. A token can be:

\begin{itemize}
\item a full word (e.g., \texttt{function}),
\item a subword piece (e.g., \texttt{func} + \texttt{tion}),
\item punctuation or symbols (e.g., \texttt{\{}, \texttt{=}, \texttt{;}),
\item or short character fragments.
\end{itemize}

Tokenization is widely used for efficiency: representing text at the character level can yield very long sequences, while representing all words as distinct tokens creates an impractically large vocabulary. Subword tokenization offers a practical trade-off by learning reusable pieces from large corpora while keeping sequence lengths and vocabulary size manageable.
...
(1) A problem is passed to a next-token generation model, which selects which agent to use in the next step.\\
(2) Each agent produces an output. The model then conditions on the history of outputs to select the next agent. This repeats until the answer agent is selected.\\
(3) After the answer agent is chosen, it summarizes what the agents found and outputs the result.
...
(1) A problem is passed to RMSearch instead of directly to the next-token generation model.\\
(2) RMSearch selects what knowledge should be retrieved in the current step. Each knowledge item specifies which agent should use it, and the system calls that agent.\\
(3) Each agent uses the retrieved knowledge and produces an output. The system then returns to (2) with the history of outputs until RMSearch chooses the answer agent.\\
(4) The answer agent summarizes the results and outputs the final response.
